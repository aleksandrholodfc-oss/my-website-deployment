      ...
      <Section background="light">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-8 max-w-4xl mx-auto">
          {content.about?.team?.map((member: any, i: number) => (
            <div key={member.id}>
              <Card hover className="group border border-slate-700/50">
                <div className="flex items-center gap-4 sm:gap-6 p-4 sm:p-6">
                  <div className="w-16 h-16 sm:w-24 sm:h-24 bg-gradient-to-br from-blue-500 to-cyan-500 rounded-2xl flex items-center justify-center shadow-lg shadow-blue-500/30 group-hover:scale-110 transition-transform duration-300 flex-shrink-0">
                    <Users className="w-8 h-8 sm:w-12 sm:h-12 text-white" />
                  </div>
                  <div>
                    <h4 className="text-lg sm:text-2xl font-bold text-white mb-1 sm:mb-2 group-hover:text-blue-400 transition-colors">
                      {member.name}
                    </h4>
                    <p className="text-blue-400 font-medium text-sm sm:text-lg">
                      {member.position}
                    </p>
                  </div>
                </div>
