...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!validation.success) {
      // If website (honeypot) is filled, it fails max(0). Deceive bot by returning success.
      if (body.website) {
    ...
    const { name, phone, email, description } = validation.data;
    ...
    const message = `
<b>Новая заявка (Заказать звонок)</b>
