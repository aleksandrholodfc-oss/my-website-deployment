...
import {
  ...
  ThumbsUp, Search, BadgeCheck, Zap,
} from 'lucide-react';
...
export default function ServicesPageClient({ content }: ServicesPageClientProps) {
  ...
  const trustItems = [
    { icon: Zap, title: 'Срочный ремонт 24/7', desc: 'Оперативно выезжаем при аварийных остановках оборудования.' },
    { icon: Search, title: 'Точная диагностика', desc: 'Проверяем утечки, электрику, компрессор, автоматику и теплообменники.' },
    { icon: BadgeCheck, title: 'Работа по регламенту', desc: 'Фиксируем причину, согласуем стоимость и тестируем оборудование.' },
  ];
    ...
    <>
          ...
          <div className="grid lg:grid-cols-[1.05fr_0.95fr] gap-10 items-center">
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }}>
              ...
              <p className="text-slate-300 text-base sm:text-lg leading-relaxed max-w-2xl mb-8">
                Ремонтируем торговое, промышленное, климатическое оборудование, автокондиционеры и рефрижераторы. Быстро определяем причину поломки, согласуем стоимость и даём гарантию на выполненные работы.
              </p>
              <div className="flex flex-col sm:flex-row gap-3 mb-8">
                <a href={`tel:${tel}`} className="w-full sm:w-auto">
                  <Button size="lg" className="w-full bg-blue-600 hover:bg-blue-500 text-white shadow-xl shadow-blue-600/25">
                    <Phone className="w-5 h-5 mr-2" /> Вызвать мастера
                  </Button>
                </a>
                <Link href="/contacts" className="w-full sm:w-auto">
                  <Button size="lg" variant="outline" className="w-full border-slate-500 text-white hover:bg-white/10">
              ...
              <div className="grid grid-cols-3 gap-3 max-w-xl">
                {[
                  ...
                  <div key={value} className="rounded-2xl border border-slate-700/70 bg-slate-900/70 p-4">
            ...
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, delay: 0.1 }} className="grid gap-4">
              {trustItems.map(({ icon: Icon, title, desc }, i) => (
                <div key={title} className="rounded-2xl border border-slate-700/70 bg-slate-900/70 p-5 shadow-xl shadow-black/20">
      ...
      <section className="bg-slate-900 py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {/* Tabs */}
          <div className="flex flex-wrap gap-2 mb-8 justify-center" role="tablist" aria-label="Категории услуг">
      ...
      <section className="bg-slate-950 py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-[0.9fr_1.1fr] gap-10 items-start">
            <div>
              <p className="text-blue-400 text-sm font-semibold uppercase tracking-widest mb-2">Преимущества сервиса</p>
              <h2 className="text-2xl sm:text-3xl font-bold text-white mb-4">Работаем как технический партнёр, а не разовый мастер</h2>
              <p className="text-slate-400 leading-relaxed">
                Мы не просто устраняем поломку, а проверяем причину отказа, состояние узлов, герметичность системы и риски повторной остановки оборудования.
              </p>
            </div>
        ...
        <section className="bg-slate-800/30 py-16">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-10">
              <p className="text-blue-400 text-sm font-semibold uppercase tracking-widest mb-2">Вопросы клиентов</p>
              <h2 className="text-2xl sm:text-3xl font-bold text-white">Часто спрашивают перед ремонтом</h2>
            </div>
            <div className="space-y-3">
              {faq.map((item: any) => {
                const open = openFaqId === item.id;
                return (
                  <div key={item.id} className="rounded-2xl border border-slate-700 bg-slate-900/60 overflow-hidden">
                    <button
                      type="button"
                    ...
                    <AnimatePresence>
                      {open && (
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                          <p className="px-5 pb-5 text-slate-400 text-sm leading-relaxed">{item.answer}</p>
                        </motion.div>
                      )}
                    </AnimatePresence>
                  </div>
            ...
            {Array.isArray(content.faq) && content.faq.length > 4 && (
              <div className="flex justify-center mt-6">
                ...
                >
                  ...
                  <ChevronDown className={`w-4 h-4 transition-transform ${showAllFaq ? 'rotate-180' : ''}`} />
                </button>
              </div>
            )}
          </div>
