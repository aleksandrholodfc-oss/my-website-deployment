...
export default function HomeClient({ content }: HomeClientProps) {
  ...
  const serviceSlug: Record<number, string> = { 1: 'trade', 2: 'industrial', 3: 'climate', 4: 'auto', 5: 'refrigerator' };
    ...
    <>
              ...
              <div className="flex flex-col flex-1 px-5 py-4">
                <ul className="flex-1 space-y-2">
                  {service.items?.map((item: string, idx: number) => (
                    <li key={idx} className="flex items-start gap-2.5 text-slate-400 text-sm group-hover:text-slate-300 transition-colors duration-300">
          ...
          return (
            <div className="space-y-5 max-w-6xl mx-auto">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {top.map((s: any, i: number) => <ServiceCard key={s.id} service={s} i={i} />)}
              </div>
              {bottom.length > 0 && (
                <div className={`grid gap-5 mx-auto ${bottom.length === 2 ? 'sm:grid-cols-2 max-w-[calc(66.666%+1.25rem)]' : 'sm:grid-cols-2 lg:grid-cols-3'}`}>
                  {bottom.map((s: any, i: number) => <ServiceCard key={s.id} service={s} i={i + top.length} />)}
                </div>
              )}
            </div>
          );
      ...
      <Section background="light">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3 max-w-4xl mx-auto">
          {content.advantages.map((adv: any, i: number) => (
            <motion.div key={adv.id} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }} className="group flex items-start gap-2.5 p-3 sm:p-4 bg-white rounded-xl border border-gray-100 hover:border-blue-200 hover:shadow-md transition-all duration-200">
              ...
              <div>
                <h4 className="text-xs sm:text-sm font-semibold text-gray-800 group-hover:text-blue-600 transition-colors leading-tight">{adv.title}</h4>
                <p className="text-gray-500 text-xs leading-snug mt-0.5 hidden sm:block">{adv.description}</p>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-wrap gap-2 sm:gap-3 mb-6">
            {content.guarantees?.map((g: any) => (
              <div key={g.id} className="flex items-center gap-2 px-3 py-1.5 bg-green-500/15 border border-green-500/30 rounded-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
            <div>
              <p className="text-xs text-blue-400 font-semibold uppercase tracking-wide mb-1">Калькулятор стоимости</p>
              <h2 className="text-xl sm:text-2xl font-bold text-white mb-3">Сколько стоит ремонт?</h2>
              <p className="text-slate-400 text-sm mb-4 leading-relaxed">
                Рассчитайте примерную стоимость за 30 секунд. Точная цена — после бесплатной диагностики на месте.
              </p>
              <ul className="space-y-1.5">
                {['Диагностика бесплатна при согласовании ремонта', 'Без скрытых платежей', 'Согласование стоимости до начала работ'].map((item, i) => (
      ...
      <Section background="light" className="pb-6 md:pb-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-5">
          <div>
            <p className="text-xs text-blue-600 font-semibold uppercase tracking-wide mb-0.5">Что говорят о нас</p>
            <h2 className="text-xl sm:text-2xl font-bold text-gray-800">Отзывы клиентов</h2>
          </div>
          {content.yandexRating && (
            <a href={content.yandexMapsUrl || 'https://yandex.ru/maps/org/federatsiya_kholoda/158193223407/reviews/'} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-red-500 text-white px-3 py-1.5 rounded-lg hover:bg-red-600 transition-colors text-xs shrink-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {content.testimonials?.slice(0, 3).map((testimonial: any, i: number) => (
            <motion.div key={testimonial.id} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="bg-white rounded-xl p-3.5 shadow-sm border border-gray-100 hover:border-blue-200 hover:shadow-md transition-all duration-200">
              <div className="flex items-center gap-0.5 mb-2">
                {[...Array(testimonial.rating)].map((_, idx) => (
                  <Star key={idx} size={12} className="text-yellow-400 fill-current" />
                ))}
              </div>
              <p className="text-gray-600 text-xs sm:text-sm leading-relaxed mb-2.5 line-clamp-3">{testimonial.text}</p>
        ...
        <div className="text-center mt-4">
          <a href={content.yandexMapsUrl || 'https://yandex.ru/maps/org/federatsiya_kholoda/158193223407/reviews/'} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-blue-600 hover:text-blue-700 text-sm font-medium transition-colors">
      ...
      <Section background="light" className="pt-6 md:pt-8">
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="text-xs text-blue-600 font-semibold uppercase tracking-wide mb-0.5">Выгодные предложения</p>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          {content.promotions?.map((promo: any, i: number) => (
            <motion.div key={promo.id} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.07 }} className="flex-1 flex items-start gap-3 p-3.5 bg-gradient-to-r from-blue-600 to-cyan-600 rounded-xl shadow-md">
              ...
              <div className="min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1.5">
                  <span className="text-xs bg-white/20 text-white px-1.5 py-0.5 rounded font-mono">{promo.code}</span>
      ...
      <Section className="py-6 md:py-8">
        <p className="text-xs text-slate-400 uppercase tracking-widest font-medium text-center mb-4">Работаем с ведущими брендами</p>
        <div className="flex flex-wrap justify-center gap-2">
          {content.partners?.map((partner: any, i: number) => (
            ...
            >
              ...
              <span className="text-slate-500 text-xs hidden sm:inline">· {partner.description}</span>
            </motion.div>
          ))}
        </div>
      </Section>
      ...
      <Section background="light">
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-5">
            <p className="text-xs text-blue-600 font-semibold uppercase tracking-wide mb-0.5">Ответы на популярные вопросы</p>
            <h2 className="text-xl sm:text-2xl font-bold text-gray-800">Часто задаваемые вопросы</h2>
          </div>
            ...
            {(showAllFaq ? content.faq : content.faq?.slice(0, 6))?.map((item: any) => (
              <div key={item.id} className="border border-gray-200 rounded-xl overflow-hidden bg-white">
                ...
                >
                  ...
                  <ChevronDown className={`w-4 h-4 text-gray-400 shrink-0 transition-transform duration-200 ${openFaqId === item.id ? 'rotate-180' : ''}`} />
                </button>
                {openFaqId === item.id && (
          ...
          {content.faq?.length > 6 && (
            <div className="text-center mt-3">
              <button onClick={() => setShowAllFaq(!showAllFaq)} className="inline-flex items-center gap-1.5 text-blue-600 hover:text-blue-700 text-sm font-medium transition-colors">
                {showAllFaq ? 'Свернуть' : `Ещё ${content.faq.length - 6} вопросов`}
                <ChevronDown className={`w-3.5 h-3.5 transition-transform ${showAllFaq ? 'rotate-180' : ''}`} />
              </button>
            </div>
          )}
        </div>
      ...
      <Section>
        <div className="bg-gradient-to-r from-blue-600 to-cyan-600 border border-blue-500/30 rounded-xl sm:rounded-2xl p-6 sm:p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-4 sm:gap-6 shadow-xl shadow-blue-500/20">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            ...
            <Button size="lg" onClick={() => setIsRequestModalOpen(true)} className="bg-blue-700 hover:bg-blue-800 text-white w-full md:w-auto text-sm sm:text-base">
              Оставить заявку
            </Button>
          </div>
        </div>
