...
export default function HomeClient({ content }: HomeClientProps) {
  ...
  const processSteps = [
    { step: '01', title: 'Заявка', desc: 'Оставьте заявку на сайте или по телефону' },
    {
      step: '02',
      title: 'Диагностика',
      desc: 'Мастер приедет и бесплатно определит неисправность',
    },
  ...
  return (
    <>
      ...
      <Section background="light">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-4 max-w-5xl mx-auto">
          {content.advantages.map((adv: any, i: number) => (
            <motion.div
              ...
              className="group flex items-start gap-3 p-4 sm:p-5 bg-slate-900/50 rounded-2xl border border-slate-700/50 hover:border-blue-500/50 hover:shadow-glow-primary transition-all duration-300"
            >
              <div className="w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-br from-blue-600 to-cyan-600 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5 shadow-lg shadow-blue-600/20">
                <CheckCircle className="w-5 h-5 text-white" />
              </div>
              <div>
                <h4 className="text-sm sm:text-base font-bold text-white group-hover:text-blue-400 transition-colors leading-tight">
                  {adv.title}
                </h4>
                <p className="text-slate-400 text-xs sm:text-sm leading-relaxed mt-1.5 hidden sm:block">
                  {adv.description}
                </p>
              </div>
            </motion.div>
      ...
      <Section background="light" className="pb-6 md:pb-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
          <div>
            <p className="text-xs text-blue-400 font-bold uppercase tracking-widest mb-1.5">
              Что говорят о нас
            </p>
          ...
          {content.yandexRating && (
            <a
              ...
              className="flex items-center gap-2 bg-red-600 text-white px-4 py-2 rounded-xl hover:bg-red-500 transition-all text-sm font-bold shadow-lg shadow-red-600/20 shrink-0"
            >
              <Star size={16} className="fill-current" />
              <span className="text-lg">{content.yandexRating}</span>
              <span className="opacity-80">· {content.yandexReviewCount || 0} отзывов</span>
            </a>
          )}
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {content.testimonials?.slice(0, 3).map((testimonial: any, i: number) => (
            <motion.div
              ...
              className="bg-slate-900/50 rounded-2xl p-6 border border-slate-700/50 hover:border-blue-500/50 transition-all duration-300"
            >
              <div className="flex items-center gap-1 mb-4">
                {[...Array(testimonial.rating)].map((_, idx) => (
                  <Star key={idx} size={14} className="text-yellow-500 fill-current" />
                ))}
              </div>
              <p className="text-slate-300 text-sm sm:text-base leading-relaxed mb-4 italic line-clamp-4">
                «{testimonial.text}»
              </p>
      ...
      <Section background="light" className="pt-6 md:pt-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <h2 className="text-2xl sm:text-3xl font-bold text-white">Акции и скидки</h2>
          </div>
        </div>
        <div className="flex flex-col sm:flex-row gap-6">
          {content.promotions?.map((promo: any, i: number) => (
            <motion.div
              ...
              className="flex-1 flex items-start gap-4 p-6 bg-gradient-to-br from-blue-600 to-cyan-600 rounded-2xl shadow-xl shadow-blue-600/20 border border-blue-400/30"
            >
              <div className="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center text-2xl shrink-0 backdrop-blur-sm">
                🎉
              </div>
              <div className="min-w-0">
                <h4 className="text-lg font-bold text-white leading-tight mb-1">{promo.title}</h4>
                <p className="text-blue-50 text-sm opacity-90 leading-relaxed mb-4">
                  {promo.description}
                </p>
                <div className="flex flex-wrap items-center gap-3">
                  <span className="text-xs bg-black/20 text-white px-3 py-1 rounded-lg font-mono font-bold tracking-wider uppercase border border-white/20">
                    {promo.code}
                  </span>
                  <span className="text-xs text-blue-100 font-medium">до {promo.validUntil}</span>
                </div>
              </div>
            </motion.div>
          ))}
      ...
      <Section background="light">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-10">
            ...
            <h2 className="text-2xl sm:text-3xl font-bold text-white">Часто задаваемые вопросы</h2>
          </div>
          <div className="space-y-3">
            {(showAllFaq ? content.faq : content.faq?.slice(0, 6))?.map((item: any) => (
              <div
                key={item.id}
                className="border border-slate-700/50 rounded-2xl overflow-hidden bg-slate-900/50 backdrop-blur-sm"
              >
                <button
                  onClick={() => setOpenFaqId(openFaqId === item.id ? null : item.id)}
                  className="w-full flex items-center justify-between gap-4 px-6 py-4 text-left hover:bg-slate-800/50 transition-all"
                >
                  <span className="text-sm sm:text-base font-bold text-white leading-snug">
                    {item.question}
                  </span>
                  <ChevronDown
                    className={`w-5 h-5 text-blue-400 shrink-0 transition-transform duration-300 ${openFaqId === item.id ? 'rotate-180' : ''}`}
                  />
                </button>
                {openFaqId === item.id && (
                  <div className="px-6 pb-5 text-sm sm:text-base text-slate-400 leading-relaxed border-t border-slate-700/50 pt-4">
                    {item.answer}
                  </div>
                )}
              </div>
