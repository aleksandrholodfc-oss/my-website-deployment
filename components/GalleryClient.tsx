      ...
      <div className="flex flex-wrap justify-center gap-2 sm:gap-3 mb-6 sm:mb-8">
        {['all', 'trade', 'industrial', 'climate', 'auto', 'refrigerator'].map((category) => (
          <button
            ...
            className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl transition-all text-xs sm:text-sm border ${
              selectedCategory === category
                ? 'bg-blue-600 border-blue-600 text-white shadow-glow-primary'
      ...
      {selectedImage && (
        ...
        >
          ...
          <div className="max-w-4xl w-full" onClick={(e) => e.stopPropagation()}>
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 sm:p-6 md:p-8 text-center shadow-2xl">
              {selectedImage.image ? (
                <Image
                  ...
                  width={800}
                  height={450}
                  className="w-full aspect-video object-cover rounded-xl mb-4 sm:mb-6"
                />
              ) : (
                <div className="aspect-video bg-gradient-to-br from-blue-500/20 to-cyan-500/20 rounded-xl flex items-center justify-center mb-4 sm:mb-6">
